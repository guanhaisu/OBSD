...
def sampling(args, pipe, name,content_image=None, style_image=None):
    if not args.demo:
...
def save_image_with_content_style(save_dir, image, content_image_pil, content_image_path, style_image_path, resolution,name):
...
if __name__=="__main__":
    ...
    image_folder = args.content_image_path
    # directorys = os.listdir(image_folder)
    # directorys_names = [item for item in directorys if os.path.isdir(os.path.join(image_folder, item))]
    # directorys_names_lengths = [len(name) for name in directorys_names]
    for directory in os.listdir(image_folder):
        if len(directory)==1:
            for filename in os.listdir(os.path.join(image_folder, directory)):
                if filename.endswith(".jpg") or filename.endswith(".png"):
                    args.content_image_path = os.path.join(image_folder,directory,filename)
                    os.makedirs(os.path.join(args.save_image_dir, directory), exist_ok=True)
                    name=os.path.join(directory, filename)
                    out_image = sampling(args=args, pipe=pipe,name=name)
        else:
            for sub_dir in directory:
                for filename in os.listdir(os.path.join(image_folder, directory,sub_dir)):
                    if filename.endswith(".jpg") or filename.endswith(".png"):
                        args.content_image_path = os.path.join(image_folder,directory,sub_dir,filename)
                        os.makedirs(os.path.join(args.save_image_dir, directory,sub_dir), exist_ok=True)
                        name=os.path.join(directory, sub_dir, filename)
                        out_image = sampling(args=args, pipe=pipe,name=name)
